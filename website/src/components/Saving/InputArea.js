//--------------------------------------------------------------------------------------------------------------------
//-------------------------------------Input area to trace investment development-------------------------------------
//--------------------------------------------------------------------------------------------------------------------
//Imports
import React, {
    useEffect
} from 'react';
import {
    FilledInput,
    InputLabel,
    InputAdornment,
    FormControl,
    TextField,
    MenuItem,
    Select,
    Button
} from '@mui/material';
import axios from 'axios';
import DesktopDatePicker from '@mui/lab/DesktopDatePicker';
import LocalizationProvider from '@mui/lab/LocalizationProvider';
import AdapterDateFns from '@mui/lab/AdapterDateFns';

function InputArea() {

    //--------------------------------------------------------------------------------------------------------------------
    //Initialization
    const [savePur, setSavePur] = React.useState([])
    const [pur, setPur] = React.useState('');
    const [costs, setCosts] = React.useState({
        amount: '',
    });
    const [date, setDate] = React.useState(new Date());

    var api = 'http://localhost:3000/'


    const handleChangePur = (event) => {
        setPur(event.target.value)
    };

    const handleChangeCost = (prop) => (event) => {
        setCosts({ ...costs, [prop]: event.target.value });
    };

    const handleChangeDate = (newValue) => {
        setDate(newValue);
    };

    function isNumeric(num) {
        return !isNaN(num)
    }


    //--------------------------------------------------------------------------------------------------------------------
    //Getting all the data
    //All purposes for savings
    function getSavePur(index) {
        axios.get(api + 'purposes/4')
            .then(response => {
                setSavePur(response.data)
            });
    }

    //--------------------------------------------------------------------------------------------------------------------
    //Send data to the database
    async function sendData() {
        if (pur !== '' && costs.amount !== '') {
            let dd = String(date.getDate()).padStart(2, '0')
            let mm = String(date.getMonth() + 1).padStart(2, '0')
            let yyyy = date.getFullYear()
            let dateFormat = dd + "-" + mm + "-" + yyyy

            const res = await axios.post(api + 'savingDev', {
                "saving_id": pur,
                "value": costs.amount,
                "date": dateFormat
            })
        } else {
            console.log("error")
        }
    }

    //--------------------------------------------------------------------------------------------------------------------
    //Get context items
    useEffect(getSavePur, [])

    //--------------------------------------------------------------------------------------------------------------------
    //Rendering
    return (
        <div id="inputFormat">
            <div className="inputObject">
                <FormControl fullWidth>
                    <InputLabel id="save">Saving</InputLabel>
                    <Select
                        labelId="save"
                        value={pur}
                        label="Saving"
                        onChange={handleChangePur}
                    >
                        {savePur?.map((item, index) => (
                            <MenuItem key={index} value={item.id}>{item.purpose}</MenuItem>
                        ))}
                    </Select>
                </FormControl>
            </div>
            <div className="inputObject">
                <FormControl fullWidth variant="filled">
                    <InputLabel htmlFor="filled-adornment-amount">Amount</InputLabel>
                    <FilledInput
                        error={isNumeric(costs.amount) ? false : true}
                        id="filled-adornment-amount"
                        value={costs.amount}
                        onChange={handleChangeCost('amount')}
                        startAdornment={<InputAdornment position="start">€</InputAdornment>}
                    />
                </FormControl>
            </div>
            <div className="inputObject">
                <LocalizationProvider dateAdapter={AdapterDateFns} >
                    <DesktopDatePicker
                        id="datePicker"
                        label="Date picker"
                        inputFormat="dd-MM-yyyy"
                        mask="__-__-____"
                        value={date}
                        onChange={handleChangeDate}
                        renderInput={(params) => <TextField {...params} />}
                    />
                </LocalizationProvider>
            </div>
            <div className="inputObject">
                <Button variant="outlined" onClick={sendData}>Submit</Button>
            </div>
        </div>
    );
}

export default InputArea;